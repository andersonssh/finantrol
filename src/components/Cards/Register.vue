<template>
    <div :class="{
      ...
      'bg-green-500': isProfit,
      'bg-red-600': !isProfit}"
    >
        <div class="rounded-full flex items-center justify-center ml-2">
            <img class="object-cover m-3 h-20 sm:h-24 rounded-full" :src="categoryImages[register.category]" alt="card 1" />
        </div>
        ...
        <div class="w-full max-w-full pl-2 pr-4 py-3 sm:py-4 sm:pl-3">
            ...
            <div class="flex">
                ...
                <div class="checkbox-wrapper-31">
                    <input type="checkbox" v-show="!isProfit">
                    <svg viewBox="0 0 35.6 35.6" v-show="!isProfit">
            ...
            <div class="text-xs text-gray-100 font-medium uppercase flex flex-wrap">
                    <div class="pr-1">
                      {{ register.category }} &bull;
                    </div>
                    <div>
                      <i class="fa-solid fa-triangle-exclamation text-yellow-400" v-show="register.isRequired"></i> {{ register.isRequired ? "OBRIGATÓRIO": "OPCIONAL"}}
                    </div>
